...
@auth.route("/login", methods=['GET', 'POST']) # Routes the login function with the get and post methods
def login():
    if request.method == 'POST': # Posting information
        email = request.form.get("email") # Defines email through the form
        password = request.form.get("password") # Defines the password through the from

        user = User.query.filter_by(email=email).first() # Defines the user
        if user:
            if check_password_hash(user.password, password): # If the password is valid
                ...
                flash('Password is incorrect.', category='error') # Return error message
        else: # If the email does not exist
            flash('Email does not exist.', category='error') # Return error message

    return render_template("login.html", user=current_user) # Render the login page

@auth.route("/sign-up", methods=['GET', 'POST']) # Routes the sign-up with the get and post methods
def sign_up():
    ...
    if form.validate_on_submit(): # Validation for the submission to sign-up
        hashed_password = generate_password_hash((form.password.data), method='sha256') # Generates a value for the password under the method of sha256
        user = User(username=form.username.data, email=form.email.data, password=hashed_password) # Defines the user
        db.session.add(user) # Adds the user
        db.session.commit() # Commits the user (and information) to the database
    ...
    return render_template('signup.html', form = form, user=current_user) # Render the sign-up page


@auth.route('/help') # Sets a route to send the user to the help page
def help():
    return render_template("help.html", user=current_user) # Takes the user to the help page

@auth.route('/files') # Sets a route to send the user to the file page\
@login_required # Requires the user to be logged in
def files():
    return render_template("file-storage.html", user=current_user) # Takes the user to the file page 
