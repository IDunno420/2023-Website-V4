...
def create_app():
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = f'sqlite:///{DB_NAME}' # Creates and defines the database under DB_NAME
    ...
    app.register_blueprint(views, url_prefix="/") # Register views python file
    app.register_blueprint(auth, url_prefix="/") # Register auth python file

    from .models import User, Post # Imports models

    with app.app_context():
        db.create_all() # Create a new database file
        print("Created Database") # Message to send if successfully creates a new database
