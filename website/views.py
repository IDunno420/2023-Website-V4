...
def create_post(): 
    if request.method == "POST": # Sets the request method to post
        ...
        if not text: # Comment must have text else send error
            ...
            # Attributes the author of the post
            db.session.add(post) # Adds the post
            db.session.commit() # Commits the text to the database
            flash('Post created!', category = 'success') 
            # If post is successful flash
            return redirect(url_for('views.home')) 
            # Redirects the user to the views page (home page)
...
# Sets the route for the deletion of posts under the ID of the post
...
def delete_post(id): 
    ...
    if not post: # If the post doesnt exist
        ...
        # Flash a message to exhibit the deletion is complete

    return redirect(url_for('views.home')) 
    # Redirect user to the home page once complete
...
def posts(username):
    ...
    # Renders out the users page if it is valid
...
# Sets a route for the liking of a post under the post ID
@login_required # Requires the user to be logged in
def like(post_id):
    ...
    # Displays the amount of likes


def save_picture(form_picture):
    path = Path("website/static/profile_pics") # Defines path
    random_hex = secrets.token_hex(8) 
    # Defines a random hex that is 8 units long string to save the image under
    _,f_ext = os.path.splitext(form_picture.filename) # Defines f_ext
    picture_fn = random_hex + f_ext 
    # Defines picture_fn under the addition of
    # the random hex value and the f_ext value
    picture_path = os.path.join(path, picture_fn) 
    # Sets the path for the database to find the image
...
# Sets a route for the account page
@login_required # Requires the user to be logged in
def account():
    ...
    if form.validate_on_submit(): # Begins validating when submitted
        if form.picture.data:
            ...
            # Shows the picture that the user has saved
        ...
        # Redirects the user to the account page (shows new changes)
    elif request.method == 'GET': 
        form.username.data = current_user.username
        form.email.data = current_user.email
    image_file = url_for('static' , filename = 'profile_pics/' + 
    current_user.image_file) 
    # Defines the image_file under a URL
    return render_template('account.html' , user = current_user,
    image_file = image_file, form = form) 
    # Returns the template of account page under the current user